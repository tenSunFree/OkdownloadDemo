...
public class SingleActivity extends BaseSampleActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        urlEditText.setText("https://99px.ru/sstorage/53/2015/05/tmb_131124_7461.jpg");

        CommonApplication.urlString = urlEditText.getText().toString();
        initSingleDownload(
    ...
    private void initSingleDownload(
            TextView statusTv, ProgressBar progressBar, View actionView, TextView actionTv) {
        initTask();
    ...
    private void initTask() {
        ...
        task = new DownloadTask.Builder(CommonApplication.urlString, parentFile)
                .setFilename(filename)
                .setMinIntervalMillisCallbackProcess(16)                                            // the minimal interval millisecond for callback progress
                .setPassIfAlreadyCompleted(false)                                                   // ignore the same task has already completed in the past.
                .build();
    }
    ...
    private void initAction(final View actionView, final TextView actionTv, final TextView statusTv,
        ...
        actionView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (!CommonApplication.urlString.equals(urlEditText.getText().toString())) {
                    CommonApplication.urlString = urlEditText.getText().toString();
                    initSingleDownload(
                            (TextView) findViewById(R.id.statusTv),
                            (ProgressBar) findViewById(R.id.progressBar),
                ...
                if (started) {
                    task.cancel();                                                                  // to cancel
                } else {
                    actionTv.setText(R.string.cancel);
                    startTask(statusTv, progressBar, actionTv);                                     // to start
                    task.setTag("mark-task-started");                                               // mark
                }
            }
        });
    }
